...
This document describes the target architecture for the `Phaser + XState` rewrite under `html/src/rewrite/`.
...
- `html/src/rewrite/games/drawPoker/`
  First concrete game implementation, including its `definition.ts`.
...
flowchart LR
    subgraph Engine
        ...
        GameCore[engine/game<br/>types + definition + viewModel]
    end

    subgraph GameModule
        DrawPoker[games/drawPoker<br/>setup + rules + definition + viewModel]
        FutureGame[games/brisca or other game]
    end
...
classDiagram
    ...
    class RewriteGameMachine
    class CardGameViewModel
    class TableScene
    class UIScene

    GameDefinition <|.. DrawPokerDefinition
    RewriteGameMachine --> DrawPokerDefinition : uses
...
`drawPoker` is the first game that implements it through:
...
1. Keep moving state decisions into game-owned `applyMove` logic instead of machine-local assumptions.
2. Replace hard-coded layout assumptions such as fixed player count and fixed hand size.
3. Introduce more generic table concepts when a second game truly needs them.
4. Add a second game only after the first game's contract is stable and understandable.
