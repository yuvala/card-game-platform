...
```ts
type ClientMessage =
  | { type: "watch-session"; sessionId: string; role?: "admin" | "player" }
  | {
...
2. Admin connects and sees the same table as today. WebSocket is now the default admin path; local mode remains available behind `transport=local`.
...
The backend currently owns one in-memory session and broadcasts `session-view` messages to connected clients. The existing `rewrite.html` admin UI uses the WebSocket session by default. The local browser session remains available as a development fallback.
...
Clients now declare a role when watching a session:

- `admin` can configure the session and send table-level game events.
...
The admin rewrite UI now uses the WebSocket server by default.
...
```text
rewrite.html?autostart=1
```

For local browser-owned fallback mode, open:

```text
rewrite.html?transport=local&autostart=1
```
...
Every `session-view` now includes a monotonically increasing `sequence` number generated by the backend.

Current behavior:

- the backend increments sequence when the session changes
- admin/player clients receive the sequence with every session view
- the remote browser session ignores stale session-view messages with a lower sequence than the latest accepted view
- remote clients include their latest sequence as `expectedSequence` on `game-event` messages
- the backend rejects game events with an older `expectedSequence` so stale UI actions do not mutate the authoritative session

Next sequence-related step:

- tighten how stale-action errors are surfaced in the UI
- decide whether some admin-only events should allow `expectedSequence` equality only, instead of only rejecting older sequences
