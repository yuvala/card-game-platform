...
function listen(server: ReturnType<typeof createRewriteWebSocketServer>["server"]): Promise<string> {
...
function waitForSessionView(
    socket: WebSocket,
    predicate: (message: Extract<RewriteServerMessage, { type: "session-view" }>) => boolean,
    timeoutMs = 4000
): Promise<Extract<RewriteServerMessage, { type: "session-view" }>> {
    return waitForServerMessage(socket, (message): message is Extract<RewriteServerMessage, { type: "session-view" }> => {
        return message.type === "session-view" && predicate(message);
    }, timeoutMs);
}

function waitForServerMessage<TMessage extends RewriteServerMessage>(
    socket: WebSocket,
    predicate: (message: RewriteServerMessage) => message is TMessage,
    timeoutMs?: number
): Promise<TMessage>;
function waitForServerMessage(
    socket: WebSocket,
    predicate: (message: RewriteServerMessage) => boolean,
    timeoutMs?: number
): Promise<RewriteServerMessage>;
function waitForServerMessage(
    socket: WebSocket,
    predicate: (message: RewriteServerMessage) => boolean,
    timeoutMs = 4000
...
function parseServerMessage(rawMessage: string): RewriteServerMessage | null {
    try {
...
main()
    .then(() => {
        console.log("rewriteWebSocketServer.test.ts passed");
    })
    .catch((error) => {
        console.error(error);
        process.exitCode = 1;
