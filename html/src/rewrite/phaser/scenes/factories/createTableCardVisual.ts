...
export function createTableCardVisual(scene: Phaser.Scene, backTextureKey: string): TableCardVisual {
    const shadow = scene.add.rectangle(5, 8, TABLE_CARD_WIDTH + 8, TABLE_CARD_HEIGHT + 8, 0x000000, 0.22);
    const stackBacks = [
        { x: -10, y: -8 },
        { x: -5, y: -4 }
    ].map((offset) => {
        const back = scene.add.image(offset.x, offset.y, backTextureKey)
    ...
    const container = scene.add.container(TABLE_CENTER_X, 392, [
        caption,
        shadow,
        ...stackBacks,
        image,
