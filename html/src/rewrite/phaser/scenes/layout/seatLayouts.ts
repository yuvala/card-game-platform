...
function createHorizontalSeat(centerX: number, labelY: number, handCenterY: number, gapX: number): SeatLayout {
    return {
        labelX: centerX,
        labelY,
        labelAlign: "center",
...
function getFallbackSeatLayout(playerIndex: number, playerCount: number): SeatLayout {
    ...
    if (seatX < TABLE_CENTER_X) {
        return createVerticalSeat(
            "left",
            Math.min(Math.max(seatY + 30, 180), 460),
            36
        );
    }
...
export function getSeatLayouts(playerCount: number): SeatLayout[] {
    ...
    return Array.from({ length: playerCount }, (_, index) => getFallbackSeatLayout(index, playerCount));
}
