...
export function drawAllCardsGallery(context: CardSandboxExampleContext): void {
    ...
    const deckIds = Object.keys(supportedDeckDefinitions) as Array<keyof typeof supportedDeckDefinitions>;

    let y = 42;

    renderLayer.add(scene.add.text(LEFT, y, "All Decks", {
        fontFamily: "Arial",
        fontSize: "20px",
        fontStyle: "700",
        color: colors.cream
    ...
    deckIds.forEach((deckId, deckIdx) => {
        ...
        renderLayer.add(scene.add.text(GRID_LEFT, y, deckDef.name, {
            fontFamily: "Arial",
            fontSize: "13px",
            fontStyle: "700",
            color: colors.cream
        }));
        renderLayer.add(scene.add.text(GRID_LEFT + 110, y + 1, `${deck.length} cards · ${suits.length} suits · ${rankCount} ranks`, {
            fontFamily: "Arial",
            fontSize: "11px",
            color: colors.dim
        }));
        ...
        firstSuitCards.forEach((card, i) => {
            ...
            renderLayer.add(scene.add.text(x, y, card.rankShortLabel, {
                fontFamily: "Arial",
                fontSize: "10px",
                color: colors.dim
            }).setOrigin(0.5, 0));
        ...
        renderLayer.add(
            scene.add.image(BACK_X, gridMidY, backKey).setDisplaySize(BACK_DISPLAY_W, BACK_DISPLAY_H)
        );
        ...
        suits.forEach((suitId, suitIdx) => {
            ...
            renderLayer.add(scene.add.text(SUIT_LABEL_RIGHT, rowCenterY, label, {
                fontFamily: "Arial",
                fontSize: "10px",
                fontStyle: "700",
                color: colors.cream
            ...
            cards.forEach((card, i) => {
                const x = cardStartX + i * (CARD_W + H_GAP) + CARD_W / 2;
                renderLayer.add(
                    scene.add.image(x, rowCenterY, getCardFaceTextureKey(card.id, skinId, "compact"))
