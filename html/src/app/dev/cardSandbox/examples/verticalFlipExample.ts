...
export const verticalFlipExample: CardSandboxExample = {
    ...
    render: (context) => {
        ...
        cardSlots.forEach(({ card, x, label, flip }) => {
            const image = scene.add.image(x, centerY, context.getFaceTexture(card, "showcase"))
                .setDisplaySize(112, 164)
                .setInteractive({ useHandCursor: true });
            const outline = scene.add.rectangle(x, centerY, 122, 174, 0x000000, 0)
                .setStrokeStyle(2, colors.gold, 0.86);
            const labelText = scene.add.text(x, centerY + 106, label, {
                fontFamily: "Arial",
                fontSize: "13px",
                fontStyle: "700",
                color: colors.dim
