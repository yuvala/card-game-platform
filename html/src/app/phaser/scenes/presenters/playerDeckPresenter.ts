...
export function createPlayerDeckVisual(
    ...
    const stackBacks = [-8, -4].map((offset) => {
        return scene.add.image(offset, offset, textureApi.getActiveBackTextureKey())
            .setDisplaySize(CARD_WIDTH, CARD_HEIGHT)
            .setVisible(false);
    });

    const image = scene.add.image(0, 0, textureApi.getActiveBackTextureKey())
        .setDisplaySize(CARD_WIDTH, CARD_HEIGHT);

    const outline = scene.add.rectangle(0, 0, CARD_WIDTH + 8, CARD_HEIGHT + 8, 0x000000, 0)
        .setStrokeStyle(1, CARD_BACK_STROKE, 0.28);

    const countText = scene.add.text(0, CARD_HEIGHT / 2 + 10, "", {
        fontFamily: TABLE_FONT_FAMILY,
        fontSize: "11px",
        color: TABLE_CREAM_DIM,
        align: "center"
    }).setOrigin(0.5, 0).setResolution(TABLE_TEXT_RESOLUTION);

    const container = scene.add.container(pos.x, pos.y, [
        ...stackBacks, image, outline, countText
    ]).setDepth(30);

    const hitTarget = scene.add.rectangle(pos.x, pos.y, CARD_WIDTH, CARD_HEIGHT, 0x000000, 0.001)
        .setDepth(31);
...
export function syncPlayerDeckPresentation(input: {
    ...
    viewModel.players.forEach((player) => {
        ...
        const visibleBackCount = isEmpty ? 0 : Math.min(stackBacks.length, Math.max(0, Math.floor(cardCount / 8)));
        stackBacks.forEach((back, i) => {
            const show = i < visibleBackCount;
            back.setVisible(show);
            back.setAlpha(show ? 0.72 - i * 0.14 : 0);
            if (show) { textureApi.applyCardBackTexture(back); }
        });
