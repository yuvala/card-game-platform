    ...
    if (layers.piles) {
        ...
        seatBadges.forEach((badge, playerId) => {
            zones.push({
                ...
                x: badge.container.x, y: badge.container.y,
                color: 0x44ff88, width: 60, height: 28
            });
        });
        ...
        ownedPileVisuals.forEach((visual, ownerId) => {
            zones.push({
                ...
                color: 0xcc66ff, width: 60, height: 88
            });
        });
        ...
        slots.forEach((slot, i) => {
            ...
            if (!isVisible) {
                if (layers.origins) {
                    zones.push({
                        ...
                        x: slot.originX, y: slot.originY,
                        color: 0x446688, width: 60, height: 88,
                        dashed: true
                    });
                }
                return;
            ...
            if (layers.slots) {
                zones.push({
                    ...
                    color: 0x66aaff, width: 60, height: 88
                });
            }
            ...
            if (layers.origins && movedFromOrigin) {
                zones.push({
                    label: `origin:${playerId}[${i}] ${fmt(slot.originX, slot.originY)}`,
                    x: slot.originX, y: slot.originY,
                    color: 0x3366cc, width: 60, height: 88,
                    dashed: true
                });
            }
        });
...
export function drawDebugZoneOverlay(input: DebugZoneOverlayInput): DebugOverlay {
    ...
    collectZones(input).forEach((zone) => {
        ...
        const text = scene.add.text(x, y - h / 2 - 3, zoneLabel, {
            fontSize: "10px",
