...
export function drawStateGallery(context: CardSandboxExampleContext): void {
    ...
    renderLayer.add(
        scene.add.text(70, 50, 'Deck states', {
            fontFamily: 'Arial',
            fontSize: '18px',
            fontStyle: '700',
            color: colors.cream,
    ...
    renderLayer.add(
        scene.add.text(70, 78, 'Static preview. Each card below represents one visual state.', {
            fontFamily: 'Arial',
            fontSize: '13px',
            color: colors.dim,
            wordWrap: { width: 360 },
    ...
    states.forEach((state, index) => {
        ...
        renderLayer.add(
            scene.add
                .image(x, 180, context.getFaceTexture(state.card, state.variant))
        ...
        if (state.label === 'selected') {
            renderLayer.add(
                scene.add
                    .rectangle(x, 180, CARD_W + 8, CARD_H + 8, 0x000000, 0)
    ...
    renderLayer.add(
        scene.add.image(backX, 180, context.getBackTexture()).setDisplaySize(CARD_W, CARD_H),
    );
    addCardCaption(context, backX, 258, 'back');
}

function addCardCaption(
    context: CardSandboxExampleContext,
    x: number,
    y: number,
    text: string,
): void {
    context.renderLayer.add(
        context.scene.add
            .text(x, y, text, {
                fontFamily: 'Arial',
                fontSize: '11px',
