...
export const collectPileExample: CardSandboxExample = {
    ...
    render: (context) => {
        ...
        tablePositions.forEach((position, index) => {
            renderLayer.add(
                scene.add
                    .rectangle(position.x, position.y, 76, 112, 0x000000, 0)
                    .setAngle(position.angle)
                    .setStrokeStyle(2, 0x78d9a0, 0.22),
            );
            renderLayer.add(
                scene.add
                    .text(position.x, cardY + 66, 'card ' + String(index + 1), {
                        fontFamily: 'Arial',
                        fontSize: '11px',
                        color: colors.dim,
                    })
        ...
        renderLayer.add(
            scene.add
                .rectangle(pileX, cardY, 78, 116, 0x000000, 0)
                .setStrokeStyle(2, colors.gold, 0.3),
        );
        renderLayer.add(
            scene.add
                .text(pileX, cardY + 66, 'pile', {
                    fontFamily: 'Arial',
                    fontSize: '11px',
                    color: colors.dim,
                })
