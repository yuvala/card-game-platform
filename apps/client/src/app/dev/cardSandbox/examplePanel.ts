...
export function drawExamplePanel(
    ...
    if (panel.title.length > 0) {
        renderLayer.add(
            scene.add.text(panel.x, panel.y, panel.title, {
                fontFamily: 'Arial',
                fontSize: '17px',
                fontStyle: '700',
                color: colors.cream,
    ...
    if (panel.description.length > 0) {
        renderLayer.add(
            scene.add.text(panel.x, panel.y + 30, panel.description, {
                fontFamily: 'Arial',
                fontSize: '13px',
                color: colors.dim,
...
export function addSandboxButton(context: CardSandboxExampleContext, input: ButtonInput): void {
    const { scene, renderLayer, colors } = context;
    const width = input.width ?? 112;
    const panel = scene.add
        .rectangle(input.x, input.y, width, 34, colors.gold, 1)
        .setInteractive({ useHandCursor: true });
    const text = scene.add
        .text(input.x, input.y, input.label, {
            fontFamily: 'Arial',
            fontSize: '13px',
            fontStyle: '700',
            color: '#10251c',
...
export function addStatusText(
    ...
    const status = context.scene.add
        .text(x, y, text, {
            fontFamily: 'Arial',
            fontSize: '12px',
            fontStyle: '700',
