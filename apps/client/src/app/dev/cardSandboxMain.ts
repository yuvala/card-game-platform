...
export function init(): void {
    ...
    const sectionButtonElements = Array.from(
        document.querySelectorAll('[data-card-sandbox-section]'),
    );
    ...
    if (
        ...
        throw new Error(
            'Card sandbox requires #card-sandbox-root, #card-sandbox-deck, #card-sandbox-replay, and section tabs.',
        );
    }

    const deckIds = Object.keys(supportedDeckDefinitions);
    deckSelectElement.replaceChildren(
        ...deckIds.map((deckId) => {
            const deckDefinition =
                supportedDeckDefinitions[deckId as keyof typeof supportedDeckDefinitions];
