...
const aliasPrefix = "@rewrite-core/";

if (!outDir || !fs.existsSync(outDir)) {
    console.error("Usage: node scripts/resolve-rewrite-core-aliases.cjs <compiled-out-dir>");
    process.exit(1);
}
...
function toRequirePath(fromFile, aliasPath) {
    const targetBase = path.join(outDir, "packages", "rewrite-core", "src", aliasPath);
