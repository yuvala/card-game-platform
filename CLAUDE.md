...
| `apps/client/src/app/` | Phaser 4 rendering and browser UI. Never imported by server or engine. |
| `apps/client/src/lobby/` | React lobby UI — Supabase auth, room creation/join, bot launch. |
...
### Phaser rendering (`apps/client/src/app/`)
...
Shell UI uses `.app*` prefix (`.appShell`, `.appSetupDock`, `.appSetupToggle`, `.appGameChoice`, `.appSegmentButton`, `.appPrimaryButton`, etc.). Defined in `apps/client/css/app.css`, generated by `apps/client/src/app/app/createGamePanel.ts`.

Lobby uses `.lobby*` prefix. Defined in `apps/client/css/lobby.css`.
