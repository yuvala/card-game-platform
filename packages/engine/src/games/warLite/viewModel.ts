...
function getWarLiteTableCards(snapshot: WarLiteViewSnapshot): CardGameViewTableCard[] {
    ...
    return snapshot.context.roundCards.map((playedCard) => {
        ...
        const isLastFaceDownCard =
            lastFaceDownCardIdByPlayer.get(playedCard.playerId) === playedCard.id;
        ...
        return {
            ...
            stackBadgeLabel:
                !playedCard.isFaceUp && isLastFaceDownCard && faceDownCount > 1
...
export function getWarLiteViewModel(
    snapshot: WarLiteViewSnapshot,
    ...
    return applyPlayerPovViewModel(
        {
            ...
            scoreLines: snapshot.context.players.map((player) => {
                const stackCount = getPileCards(
                    snapshot.context.piles,
                    getWarLiteHandPileId(player.id),
                ).length;
                const wonCount = getPileCards(
                    snapshot.context.piles,
                    getWarLiteCapturePileId(player.id),
                ).length;
            ...
            players: snapshot.context.players.map((player) => {
                ...
                return {
                    ...
                    metaLabel:
                        String(stackCards.length) + ' stack | ' + String(player.score) + ' wins',
                    hand: [],
                    deckPile: {
                        ...
                        topCard: topStackCard
                            ? {
                                  id: topStackCard.id,
                                  label: topStackCard.displayLabel,
                                  isFaceUp: false,
                              }
                            : null,
                    },
            ...
            controls: {
                ...
                canPlay:
                    snapshot.matches('battleReady') && Boolean(nextRevealPlayer || isWarFaceDown),
            ...
            outcome: isGameOver
                ...
                : null,
            primaryAction:
                isBattleReady && (nextRevealPlayer || isWarFaceDown)
                    ? {
                          label: isWarFaceDown
                              ? 'Place War Cards'
                              : isWarReveal
