...
function getPreviewCard(context: PokerLiteContext) {
    ...
    if (context.selectedCardId) {
        return (
            getPileCards(context.piles, getPokerLiteHandPileId(player.id)).find((card) => {
...
export function setTurnStatus(context: PokerLiteContext): PokerLiteContext {
    ...
    return {
        ...context,
        statusText:
            'Round ' + context.round + ': ' + currentPlayer.name + ' is up. Select a card to play.',
    };
}

export function selectCard(context: PokerLiteContext, cardId: string): PokerLiteContext {
    const currentPlayer = getCurrentPlayer(context);
    const clickedCard = getPileCards(context.piles, getPokerLiteHandPileId(currentPlayer.id)).find(
        (card) => {
...
export function queuePlayedCardWithEffects(context: PokerLiteContext): {
...
export function finalizeTurn(context: PokerLiteContext): PokerLiteContext {
    ...
    if (context.roundCards.length < context.players.length) {
        return {
            ...context,
            statusText:
                lastPlayedCard.playerName + ' committed ' + lastPlayedCard.card.displayLabel + '.',
        };
    }
    ...
    const winningLabel = winningCards.map((playedCard) => playedCard.playerName).join(', ');
