...
function createHiddenPreviewCards(
...
export function getBriscaLiteViewModel(
    snapshot: BriscaLiteViewSnapshot,
    ...
    return applyPlayerPovViewModel(
        {
            ...
            players: snapshot.context.players.map((player) => {
                ...
                const revealHand = viewerId
                    ? player.id === viewerId
                    : (snapshot.matches('playerTurn') || snapshot.matches('animatingPlay')) &&
                ...
                return {
                    ...
                    metaLabel: String(handCount) + ' cards | ' + String(player.score) + ' tricks',
                    hand: revealHand
                        ? faceUpCards
