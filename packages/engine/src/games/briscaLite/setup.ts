...
function resolveCardsPerPlayer(
    ...
    return Math.max(
        1,
        Math.min(
            briscaLiteConfig.openingHandSize,
            requestedCardsPerPlayer,
            supportedCardsPerPlayer,
        ),
    );
}
...
export function dealOpeningHands(context: BriscaLiteContext): {
    ...
    for (let cardIndex = 0; cardIndex < context.cardsPerPlayer; cardIndex += 1) {
        context.players.forEach((player) => {
            ...
            const nextState = moveTopCardBetweenPiles(piles, BRISCA_LITE_STOCK_PILE_ID, toPileId);
